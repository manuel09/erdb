...
const GENERATED_LOGO_VARIANT_CACHE_TTL_MS = parseCacheTtlMs(
...
const parseDisplayRatingValue = (value: string) => {
...
const buildGeneratedLogoVariantDataUrl = async (
  title: string,
  logoFontVariant: LogoFontVariant,
  primaryColor: string,
  secondaryColor: string,
...
const buildRankingBadgeSvg = (value: string, label: string, iconDataUri?: string | null, noBox = false, scale = 1.0) => {
...
const renderWithSharp = async (
  ...
  return await measurePhase(phases, 'render', async () => {
    ...
    if (input.imageType === 'poster' && input.rankingBadge) {
      ...
      if (lastPosterQualityTopY > 0 && top < lastPosterQualityBottomY && top + renderedHeight > lastPosterQualityTopY) {
